...
if not exist "server\.venv" (
  echo [오류] 파이썬 가상환경 없음: server\.venv
  echo   먼저 분석서버_터널_실행.bat 를 한 번 실행해 환경을 만드세요.
  pause & exit /b 1
)
...
findstr /b /c:"VITE_SUPABASE_ANON_KEY=" .env.local | findstr /r /c:"=.*[A-Za-z0-9]" >nul
if errorlevel 1 (
  echo [주의] .env.local 의 VITE_SUPABASE_ANON_KEY 가 비어 있습니다.
  echo   이 값이 없으면 로그인/저장이 안 되고 설정 안내 화면만 나옵니다.
  echo   Supabase 대시보드 - Project Settings - API - anon public 키를 넣으세요.
  echo.
  pause
)
...
start "이음 분석서버(로컬)" cmd /k "cd /d "%~dp0server" && call .venv\Scripts\activate.bat && python app.py"

rem 서버가 /health 에 응답할 때까지 대기
set /a _tries=0
:waithealth
...
start "이음 프론트(로컬)" cmd /k "cd /d "%~dp0" && npm run dev"
